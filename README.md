...
## Common Workflows
- **Reuse embeddings; retrain and re-evaluate** – when you tweak model hyperparameters but the accession universe is unchanged.
  - `bash scripts/clean_all.sh --yes --results`
  - `bash scripts/run_all.sh config.yaml`  *(embedding step is skipped because the cache remains)*

- **Recluster only** – when you change identity thresholds or want fresh folds with the same sequences.
  - `bash scripts/clean_all.sh --yes --clusters`
  - `bash scripts/run_all.sh config.yaml`  *(rebuilds clusters/splits; embeddings are reused)*

- **Recluster plus re-embed** – required after adding/removing sequences (e.g., TrEMBL top-up, new filters) so every accession has an embedding.
  - `bash scripts/clean_all.sh --yes --clusters`
  - `bash scripts/run_all.sh config.yaml --force-embed`

- **Reuse trained checkpoint; rerun evaluation only** – for new thresholds/calibration or score aggregation without retraining.
  - Episodic: `python -m src.eval_protonet -c config.yaml`
  - Global support: `python scripts/eval_global.py --config config.yaml --split test`

- **Generate fresh benchmark visuals** – after `run_identity_benchmark.py` completes and you want plots/PDF.
...
- **Compare two configs (A/B)** – to evaluate Swiss-Prot-only vs mixed data, or other config tweaks.
  - `bash scripts/run_all.sh config.variantA.yaml --force-embed`
  - `bash scripts/run_all.sh config.variantB.yaml --force-embed`
...
Outputs (default `--out_dir report/graphs`):
- `accuracy_vs_threshold.png` (K=1, mean ± 95% CI)
...
- `identity_benchmark_report.pdf` (bundled multi-page PDF with summary + all plots; written to `report/`)

Interpretation tips:
- Accuracy generally increases with higher identity thresholds (task becomes easier as train/test are more similar).
